...
set_sender_threads (int value){
    ...
    printf("Setting sender_thread to %d\n", options.sender_thread);
...
static int
set_receiver_xstreams (int value){
    if (MIN_NUM_THREADS > value || value>=MAX_NUM_THREADS) {
        return -1;
    }
...
set_xstreams (char *val_str)
{
    ...
    if (!count) {
        retval = set_receiver_xstreams(atoi(val_str));
        ...
        if (val1 && val2) {
            retval = set_sender_xstreams(atoi(val1));
            if(retval==-1){
                return retval;
            }
            retval = set_receiver_xstreams(atoi(val2));
        } 
